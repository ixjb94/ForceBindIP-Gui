...
$ForceBindIP = GUICreate("ForceBindIP - By Maksim4081862", 627, 429, -1, -1)
...
; Изменено: уменьшена ширина Input, добавлена кнопка обзора
$Input_appAddress = GUICtrlCreateInput("Application Address", 32, 72, 513, 21)
$Button_browse = GUICtrlCreateButton("...", 550, 72, 30, 21) ; Кнопка "..." для выбора файла
...
GUICtrlCreateGroup("", -99, -99, 1, 1)
$Label_copyRight = GUICtrlCreateLabel("Maksim4081862", 520, 400, 84, 17)
...
While 1
    ...
    Switch $nMsg
        ...
        ; === НОВОЕ: Обработчик кнопки обзора ===
        Case $Button_browse
            BrowseApp()
...
; === НОВАЯ ФУНКЦИЯ: Открытие диалога выбора файла ===
Func BrowseApp()
    Local $sFile = FileOpenDialog("Select Application", @ProgramFilesDir, "Executable files (*.exe)|All files (*.*)", $FD_FILEMUSTEXIST)
    If Not @error Then
        GUICtrlSetData($Input_appAddress, $sFile)
    EndIf
EndFunc
...
Func GetInternet()
    ...
    ; ИСПРАВЛЕНО: добавлены \\ после winmgmts:
    $objWMIService = ObjGet("winmgmts:\\" & $strComputer & "\root\CIMV2")
    
    ; Проверка на ошибку
    If @error Then
        MsgBox(16, "WMI Error", "Failed to connect to WMI service.")
        Return
    EndIf
